...
async def trigger_run_agent(body: RunAgentRequest):
    """Start a fresh research-assistant workflow and return its UUID + output.

    Always starts a new workflow — retrieve-before-start isn't applicable here
    because the UUID is generated by DBOS, not supplied by the caller.
    Use POST /resume-workflow to reconnect to an existing run.
    """
...
async def resume_workflow(body: ResumeWorkflowRequest):
    ...
    Uses DBOS.retrieve_workflow_async — the workflow must already exist in DBOS.
    If the workflow is still running this will block until it completes.
