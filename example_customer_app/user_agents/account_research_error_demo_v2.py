...
OUTREACH_RECEIPT_DIR = Path("/tmp/amber-account-research-outreach-receipts-v2")
...
@step(name="normalize_account_request_v2")
...
@step(name="determine_workflow_branch_v2")
...
@step(name="stage_outreach_payloads_v2")
...
@function_tool
@step(name="fetch_pricing_signals_v2")
...
@function_tool
@step(name="get_known_tech_stack_v2")
...
@function_tool
@step(name="search_recent_news_v2")
...
@function_tool
@step(name="search_competitor_positioning_v2")
...
@function_tool
@step(name="search_hiring_signals_v2")
...
@step(name="scrape_deep_competitive_signals_v2")
...
@function_tool(failure_error_function=None)
@step(name="send_account_brief_email_v2")
def send_account_brief_email(
    ...
    logger.info(
        "account-research-error-demo-v2 brief email sent to %s for company=%s",
        ae_email,
        company,
    )
...
async def _run_standard_research(account: dict[str, Any]) -> dict[str, str]:
    ...
    for _ in range(MAX_RESEARCH_STEPS):
        ...
        logger.info("account-research-error-demo-v2 selected module: %s", next_action)
    ...
    for action in missing:
        logger.info("account-research-error-demo-v2 guardrail selected module: %s", action)
        module_result = await agent_runner(
...
@register_agent(name="account-research-error-demo-v2")
async def account_research_error_demo_v2(request: str) -> str:
    ...
    if workflow_branch == "enterprise":
        logger.info(
            "account-research-error-demo-v2 entering enterprise branch for company=%s",
            account["company"],
        )
    ...
    logger.info("account-research-error-demo-v2 final output:\n%s", final_output)
    print(f"account-research-error-demo-v2 final output:\n{final_output}")
    return final_output
